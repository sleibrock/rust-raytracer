...
fn main() {
    ...
    let settings = Settings::new("test.ppm")
        ...
        .height(500)       // height of the img
        .aa_samples(50)   // how many rays per pixel for antialiasing
        .depth_limit(50);  // what the maximum recursion limit should be
    
    // inject spheres into the scene
    w.push(new_sphere( 0.0, 0.0, -1.0, 0.5, metal(0.3, 0.5, 0.7)));
    w.push(new_sphere( 1.0, 0.0, -1.0, 0.5, metal(0.5, 0.7, 0.1)));
    w.push(new_sphere(-1.0, 0.0, -1.0, 0.5, metal(0.1, 0.1, 0.1)));
